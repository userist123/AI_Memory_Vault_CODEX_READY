...
def test_invariant_replan_triggers_fresh_tier():
    ...
    new_plan = ActivePlan(
        "update my notes",
        [{"step": 1, "action": "delete_canonical", "query": "update my notes", "description": "Escalated recovery requiring review"}],
    )
...
def test_executive_replan_rederives_and_dispatches_new_high_risk_tier():
    ...
    class RecordingOrchestrator:
        ...
        def route_and_dispatch(self, principal, query, context, **kwargs):
            self.calls.append({"principal": principal, "query": query, "context": context, **kwargs})
            return {"orchestration_history": [], "total_context_used": len(context), "status": "completed"}
    ...
    class PlannerStub:
        ...
        def replan(self, goal, context, failed_action, error):
            return ActivePlan(goal, [{"step": 1, "action": "delete_canonical", "query": goal, "description": "Escalated recovery requiring review"}])

    executive.planner = PlannerStub()
    executive.active_plan = ActivePlan(
        "update my notes",
        [{"step": 1, "action": "search", "query": "update my notes", "description": "Initial read-only attempt"}],
    )
