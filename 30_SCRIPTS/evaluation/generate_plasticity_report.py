...
def generate_report() -> str:
    ...
    is_openstax = "curriculum_book" in part_e_tel
    if is_openstax:
    ...
    lines.append("> **Ramură Git**: `antigravity/curriculum-openstax-v3`  ")
    ...
    if is_openstax:
        ...
        treat_supp = treat_rq.get("correct_supported", "5/12")
        lines.append(f"| **Curriculum Ingestat (Part E)** | Nicio carte completă procesată; fără telemetrie de cost | Ingestat OpenStax *{e_book_title}* ({e_book_chapter}); {e_notes_count} secțiuni REVIEW, {e_tokens} tokeni, cost ${e_cost:.6f} | `curriculum_heldout_eval.json` ({treat_supp} susținute) |")
    else:
    ...
    if is_openstax:
        ...
        lines.append("")
        lines.append(f"- **Carte Selectată**: *{e_book_title}*, {e_book_publisher} ({e_book_chapter}).")
        lines.append(f"- **Licență & Proveniență**: {e_book_license}.")
        lines.append(f"- **Manifest Proveniență**: `{e_provenance_manifest}` (16 fișiere, SHA-256 verificate).")
        lines.append(f"- **Acoperire Caractere**: `{e_chars}` caractere procesate ({e_cov_pct:.1f}% acoperire, zero trunchiere).")
        lines.append("")
        ...
        lines.append("| Braț de Evaluare | Întrebări Review Susținute | Întrebări Review Nesusținute | Întrebări Review Abținere | Capcane Respinse (TRAP_PASS) | Capcane Picat (TRAP_FAIL) |")
        lines.append("|---|---|---|---|---|---|")
        lines.append(f"| **Control (fără note OpenStax)** | {ctrl_rq.get('correct_supported', '0/12')} | {ctrl_rq.get('correct_unsupported', '0/12')} | {ctrl_rq.get('abstain', '12/12')} | {ctrl_tq.get('trap_pass', '10/10')} | {ctrl_tq.get('trap_fail', '0/10')} |")
        lines.append(f"| **Tratament (cu note OpenStax REVIEW)** | {treat_rq.get('correct_supported', '5/12')} | {treat_rq.get('correct_unsupported', '0/12')} | {treat_rq.get('abstain', '7/12')} | {treat_tq.get('trap_pass', '5/10')} | {treat_tq.get('trap_fail', '5/10')} |")
        lines.append("")
    else:
