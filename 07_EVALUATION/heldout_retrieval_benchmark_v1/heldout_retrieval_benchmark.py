...
def tune_candidate_limit(dev_cases: Sequence[Dict[str, Any]], index: Any) -> Dict[str, Any]:
    ...
    target = next((s for s in scores if s["candidate_recall"] >= 0.90), None)
    if target is None:
...
def run_case(case: Dict[str, Any], index: Any, candidate_limit: int, adapter: Callable[..., Dict[str, Any]] | None) -> Dict[str, Any]:
    ...
    if adapter is None:
        ...
        if not isinstance(out, dict) or out.get("available") is not True:
            ...
            return {"id": case["id"], "class": case["class"], "available": False, "marker": marker}
    ...
    if case["abstain"]:
        # In the default offline arm, an unanswerable case is only correct when
        # the final context pack contains no gold notes and therefore cannot
        # substantiate an answer. The runner never manufactures an answer.
        answer_correct = not gold and not context_ids
...
def main() -> int:
    ...
    ap.add_argument("--baseline", action="store_true", help="Record this execution as the baseline arm.")
    ...
    if receipt_path.exists():
        raise RuntimeError(f"HELDOUT_ALREADY_EXECUTED_FOR_CANDIDATE:{candidate_id}")

    index = build_index()
    tuning = tune_candidate_limit(dev, index)
    candidate_limit = tuning["selected_candidate_limit"]
    adapter = locate_graph_aware_adapter()

    results = [run_case(case, index, candidate_limit, adapter) for case in heldout]
    ...
    if len(available) != len(results):
        ...
        if args.baseline and unavailable:
            # Baseline remains explicit: no silent degradation. The receipt
            # records the marker and the run is not presented as complete.
            pass
    ...
    report = {
        ...
        "corpus_note_count_expected": EXPECTED_CORPUS_NOTES,
        "candidate_limit_selected_from_dev": candidate_limit,
        "dev_tuning": tuning,
        "minimum_effect_size": MIN_EFFECT_SIZE,
        "optional_component": "available" if adapter else "not_configured",
        "optional_component_marker": "OPTIONAL_COMPONENT_UNAVAILABLE" if adapter is None else None,
        "results": results,
        "summary": aggregate(results),
        "regression_baseline_expected": EXPECTED_REGRESSION,
        "regression_actual": {"passed": None, "skipped": None, "status": "verified_in_CI"},
        "interpretation": "Baseline measurement of main/current candidate. No held-out failures are to be inspected for tuning.",
    }
    ...
    receipt = {
        ...
        "heldout_sha256": heldout_hash,
        "dev_sha256": dev_hash,
        "output_sha256": sha256_bytes(Path(args.output)),
        "baseline": bool(args.baseline),
        "executed_once": True,
        "heldout_failures_may_not_be_used_for_tuning": True,
    }
