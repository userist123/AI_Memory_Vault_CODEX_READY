"""
Trading Bot — Chart Widget
Candlestick chart with EMAs, Bollinger, RSI, MACD, Volume.
"""
import pyqtgraph as pg
from pyqtgraph import PlotWidget, mkPen
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QCheckBox
from PyQt6.QtCore import Qt, QRectF
from PyQt6.QtGui import QPainter, QColor, QBrush, QPen as QPen2
import numpy as np
import pandas as pd

pg.setConfigOption("background", "#0a0e17")
...
class CandlestickItem(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        for idx, o, h, l, c in self.data:
            color = QColor("#00c853") if c >= o else QColor("#ff1744")
            p.setPen(QPen2(color, 1))
...
class ChartWidget(QWidget):
    ...
    def _make_plot(self, height=None, label=None):
        ...
        if label:
            plot.setLabel("left", label, color="#8b949e", size="9pt")
        return plot
    ...
    def _setup_ui(self):
        ...
        self.cb_vol = QCheckBox("Volum"); self.cb_vol.setChecked(True)
        self.cb_sr = QCheckBox("S/R Levels"); self.cb_sr.setChecked(True)
        for cb in [self.cb_ema20, self.cb_ema50, self.cb_ema200, self.cb_bb, self.cb_vol, self.cb_sr]:
        ...
        self.rsi_plot = self._make_plot(height=90, label="RSI")
        self.macd_plot = self._make_plot(height=90, label="MACD")

        for p in [self.vol_plot, self.rsi_plot, self.macd_plot]:
            p.setXLink(self.price_plot)

        self.rsi_plot.addLine(y=70, pen=mkPen("#ff1744", width=1, style=Qt.PenStyle.DashLine))
        self.rsi_plot.addLine(y=30, pen=mkPen("#00c853", width=1, style=Qt.PenStyle.DashLine))
    ...
    def load_data(self, df: pd.DataFrame, symbol: str, supports=None, resistances=None):
        self.df = df.copy()
        self.current_symbol = symbol
        self.sr_lines = (supports or [], resistances or [])
        self.refresh_chart()
    ...
    def refresh_chart(self):
        ...
        self.rsi_plot.addLine(y=70, pen=mkPen("#ff1744", width=1, style=Qt.PenStyle.DashLine))
        self.rsi_plot.addLine(y=30, pen=mkPen("#00c853", width=1, style=Qt.PenStyle.DashLine))
        self.macd_plot.addLine(y=0, pen=mkPen("#444", width=1, style=Qt.PenStyle.DashLine))

        candle_data = list(zip(x, df["open"].values, df["high"].values, df["low"].values, df["close"].values))
        ...
        try:
            ...
            if self.cb_bb.isChecked():
                ...
                self.price_plot.plot(x, bb.bollinger_hband().values, pen=mkPen("#546e7a", width=1, style=dash))
                self.price_plot.plot(x, bb.bollinger_lband().values, pen=mkPen("#546e7a", width=1, style=dash))
                self.price_plot.plot(x, bb.bollinger_mavg().values, pen=mkPen("#546e7a", width=1))
        except Exception:
            pass

        # Support / Resistance lines
        if self.cb_sr.isChecked() and self.sr_lines:
            supports, resistances = self.sr_lines
            for s in supports[:3]:
                self.price_plot.addLine(y=s, pen=mkPen("#00c853", width=1, style=Qt.PenStyle.DotLine))
            for r in resistances[:3]:
                self.price_plot.addLine(y=r, pen=mkPen("#ff1744", width=1, style=Qt.PenStyle.DotLine))

        # Volume
        if self.cb_vol.isChecked():
            for i in range(n):
                color = "#00c853" if df["close"].values[i] >= df["open"].values[i] else "#ff1744"
                bar = pg.BarGraphItem(x=[x[i]], height=[df["volume"].values[i]], width=0.6,
        ...
        try:
            ...
            for i, h in enumerate(hist):
                color = "#00c853" if h >= 0 else "#ff1744"
