/**
 * JARVIS Web Ecosystem — Automated smoke test
...
async function runSmoke(serverProcess) {
  try {
    ...
    for (const landmark of LANDMARKS) {
      root.body.includes(landmark)
        ? pass(`HTML landmark: ${landmark}`)
        : fail(`Missing HTML landmark: ${landmark}`);
    }
    ...
    for (const endpoint of JS_ENDPOINTS) {
      ...
      contentType.includes('application/javascript')
        ? pass(`${endpoint} MIME is JavaScript`)
        : fail(`${endpoint} wrong MIME: ${contentType}`);
    }

    const missing = await get(`${BASE}/nonexistent_file_xyz.js`);
    missing.status === 404 ? pass('404 handling') : fail(`404 handling returned ${missing.status}`);

    const traversal = await get(`${BASE}/%2e%2e/%2e%2e/README.md`);
    traversal.status === 403 ? pass('Path traversal blocked') : fail(`Path traversal returned ${traversal.status}`);
  } catch (error) {
...
serverProcess.stdout.on('data', (data) => {
  ...
  if (!ready && output.includes('HTTP Server:')) {
    ready = true;
