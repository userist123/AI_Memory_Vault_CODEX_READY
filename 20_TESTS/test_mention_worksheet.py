...
def test_a_term_the_book_names_differently_is_still_found():
    """Zero is the worst possible wrong answer: it says there is nothing to
    look at.

    Three Soar concepts came back with zero mentions while being genuinely
    present — the book writes "appraisal", "SVS" and "feelings", never
    "cognitive appraisal", "spatial visual system" or "feeling state".
    """
    chunks = [
        _chunk(0, "The appraisal process assigns an emotional value to the "
                  "current situation before the agent selects an operator."),
        _chunk(1, "Nothing relevant is discussed in this particular section."),
    ]
    found = W.find_mentions(chunks, "cognitive appraisal")
    assert [m["chunk_index"] for m in found] == [0]
    assert found[0]["loose_match"] is True, "a weaker hit must say so"
    assert found[0]["matched_on"] == "appraisal"


def test_the_fallback_picks_the_rarest_word_not_the_grammatical_head():
    """Falling back to the head is worse than useless here.

    The head of "feeling state" is "state", which appears in 87 of Soar's 114
    sections and is its own concept in that book; the head of "spatial visual
    system" is "system". Both would hand back most of the book instead of a
    short list.
    """
    chunks = [_chunk(i, "The state of the system is updated each cycle.")
              for i in range(8)]
    chunks.append(_chunk(8, "A feeling is generated by the appraisal module."))

    assert W.most_distinctive_word("feeling state", chunks) == "feeling"
    found = W.find_mentions(chunks, "feeling state")
    assert [m["chunk_index"] for m in found] == [8], (
        "the rarest word narrows the search; the head would return everything"
    )


def test_a_single_word_concept_has_no_fallback():
    """There is nothing rarer to fall back to, and guessing would be worse."""
    assert W.most_distinctive_word("chunking", []) is None
    assert W.find_mentions([_chunk(0, "nothing here")], "chunking") == []
